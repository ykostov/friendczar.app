defmodule FriendczarWeb.PersonResetPasswordController do
  ...
  def create(conn, %{"person" => %{"email" => email}}) do
    if person = Authentication.get_person_by_email(email) do
      Authentication.deliver_person_reset_password_instructions(
        person,
  ...
  def edit(conn, _params) do
    render(conn, "edit.html", changeset: Authentication.change_person_password(conn.assigns.person))
  end
  ...
  def update(conn, %{"person" => person_params}) do
    case Authentication.reset_person_password(conn.assigns.person, person_params) do
      {:ok, _} ->
        conn
  ...
  defp get_person_by_reset_password_token(conn, _opts) do
    ...
    if person = Authentication.get_person_by_reset_password_token(token) do
